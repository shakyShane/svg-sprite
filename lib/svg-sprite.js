 ...
 * @return {SVGSprite}
 */
SVGSprite.prototype.createSprite = function () {
    this._reset();
    return this;
};
...
SVGSprite.prototype._addToSprite = function (svgID, svgInfo, svgPseudo, isLastSVG) {
    ...
    this.data.svg.push({
        ...
        selector: (svgPseudo || (cls.length > 1)) ? [
            ...
            {
                expression: ((cls.length > 1) ? cls.join('\\:') : (cls[0] + '\\:regular')),
                raw: ((cls.length > 1) ? cls.join(':') : (cls[0] + ':regular')),
                first: false,
                last: true
            }
        ] : [
        ...
        dimensions: {
            selector: (cls.length == 1) ? [
                ...
                {
                    expression: cls[0] + '-dims:' + cls[1],
                    raw: cls[0] + '-dims:' + cls[1],
                    first: true,
                    last: false
                },
                {
                    expression: cls[0] + '\\:' + cls[1] + '-dims',
                    raw: cls[0] + ':' + cls[1] + '-dims',
                    first: false,
                    last: true
                }
            ],
...
SVGSprite.prototype._processFile = function (id, ns, filePath, contents, callback) {
    if (this._options.verbose > 1) {
        console.log(util.format('·· Processing SVG image "%s"', path.basename(filePath)));
    }
...
function createSprite(options, callback) {
    ...
    try {
        ...
        return new SVGSprite(options).createSprite(callback);
    } catch (error) {
        return error;
    }
}
